...
from EnvDyadTraining import DyadTrackingEnv
...
class DataSaver:
    ...
    """
    def __init__(self, save_dir="./dual_tracking_data"):
...
def collect_dual_tracking_data(total_timesteps=100000, sequence_length=10, auto_save=True, save_dir="./dual_tracking_data"):
    """
    Collecting the sequensial data from dyad tracking env
...
def collect_multiple_episodes(num_episodes=100, timesteps_per_episode=2000, sequence_length=10, 
                            auto_save=True, save_dir="./dual_tracking_data"):
    """
    ...
    for episode in range(num_episodes):
        ...
        env = DyadTrackingEnv(dt=0.01)
        episode_dataset1, episode_dataset2, _ = collect_dual_tracking_data(
        ...
        print(f"episode{episode+1}complete: Agent1={len(episode_dataset1)}, Agent2={len(episode_dataset2)} sequences")    
    print(f"\nall episodes complete!")
    print(f"total data size: Agent1={len(all_dataset1)}, Agent2={len(all_dataset2)} sequences")
    # autosaving all data
    saved_key = None
    if auto_save and len(all_dataset1) > 0:
        saved_key = saver.save_datasets(
            all_dataset1, all_dataset2,
            'dual_tracking', 'multi_episode',
...
def main():
    ...
    print("2. multi-episodes（100 × 2,000ステップ）")
    ...
    while True:
        try:
            ...
            if choice == '1':
                ...
                dataset1, dataset2, key = collect_dual_tracking_data(
                    total_timesteps=100000,  # 100,000 → 100,000に増加
                    sequence_length=10
                )
                print(f"保存キー: {key}")
                break
