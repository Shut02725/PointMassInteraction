...
class DyadTrackingEnv:
    ...
    def step(self):
        ...
        # 総入力（制御 + 相互作用力 + 外力）
        tau1 = self.agent1_control + F_interaction_noisy + self.agent1_force
        tau2 = self.agent2_control - F_interaction_noisy + self.agent2_force
        ...
        self.agent1_pos_error = torch.tensor([agent1_state_estimated[0], agent1_state_estimated[3]], device=device)
        self.agent1_vel_error = torch.tensor([agent1_state_estimated[1], agent1_state_estimated[4]], device=device)
        self.agent1_acc_error = torch.tensor([agent1_state_estimated[2], agent1_state_estimated[5]], device=device)

        self.agent2_pos_error = torch.tensor([agent2_state_estimated[0], agent2_state_estimated[3]], device=device)
        self.agent2_vel_error = torch.tensor([agent2_state_estimated[1], agent2_state_estimated[4]], device=device)
        self.agent2_acc_error = torch.tensor([agent2_state_estimated[2], agent2_state_estimated[5]], device=device)

        print("estimation:", self.agent1_pos_error, "\ntrue:", self.agent1_pos - self.target_pos)
