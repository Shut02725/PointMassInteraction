...
class ForwardDynamicsLSTM(nn.Module):
    def __init__(self, input_dim=12, output_dim=6, hidden_dim=512, 
                 num_layers=2, dropout=0.0, seed=None):
        super().__init__()
...
def test_model():
    ...
    # 異なるRNNタイプでモデルを作成
    models = {
        'LSTM': ForwardDynamicsLSTM(input_dim, output_dim, seed=42),
    }
    ...
    for rnn_type, model in models.items():
        ...
        if rnn_type == 'LSTM':
            print(f"Hidden state shape: h={init_hidden[0].shape}, c={init_hidden[1].shape}")
        else:
