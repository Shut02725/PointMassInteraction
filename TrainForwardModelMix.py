...
def train_agent_model(model, 
                     dyad_dataset_training, dyad_dataset_validation,
                     solo_dataset_training, solo_dataset_validation,
                     normalizer_dyad, normalizer_solo,
                     epochs=200, batch_size=32, lr=1e-3, agent_name="Agent", 
                     prediction_mode="last_step", seed=42, verbose=True):
    """
    2つのデータセットを交互に使用する学習関数
    偶数epoch: Dyadデータセット
    奇数epoch: Soloデータセット
    """
    # 正規化されたデータセットを作成
    train_dataset_dyad = normalize_dataset(dyad_dataset_training, normalizer_dyad)
    val_dataset_dyad = normalize_dataset(dyad_dataset_validation, normalizer_dyad)
    train_dataset_solo = normalize_dataset(solo_dataset_training, normalizer_solo)
    val_dataset_solo = normalize_dataset(solo_dataset_validation, normalizer_solo)
    ...
    if verbose:
        ...
        print(f"  Dyad訓練データ: {len(train_dataset_dyad)} samples, 検証データ: {len(val_dataset_dyad)} samples")
        print(f"  Solo訓練データ: {len(train_dataset_solo)} samples, 検証データ: {len(val_dataset_solo)} samples")
    ...
    for epoch in range(epochs):
        # epochが偶数ならDyad、奇数ならSoloを使用
        if epoch % 2 == 0:
            train_dataset = train_dataset_dyad
            val_dataset = val_dataset_dyad
            dataset_type = "Dyad"
        else:
            train_dataset = train_dataset_solo
            val_dataset = val_dataset_solo
            dataset_type = "Solo"
        ...
        if verbose and (epoch % 40 == 0 or epoch == epochs - 1):
            ...
            print(f"  Epoch {epoch+1:3d}/{epochs} [{dataset_type}]: Train Loss={avg_train_loss:.6f}, "
...
def save_model_and_results(model, normalizer_dyad, normalizer_solo, 
                          train_losses, val_losses, train_acc, val_acc, 
                          agent_name, save_dir, seed):
    """学習済みモデルと結果を保存（2つのnormalizerを保存）"""
    ...
    # Dyad用normalizerの保存
    normalizer_dyad_path = os.path.join(model_save_dir, f"{agent_name}_normalizer_dyad.pkl")
    normalizer_dyad.save(normalizer_dyad_path)
    
    # Solo用normalizerの保存
    normalizer_solo_path = os.path.join(model_save_dir, f"{agent_name}_normalizer_solo.pkl")
    normalizer_solo.save(normalizer_solo_path)
...
def main():
    # 固定された設定
    save_dir_dyad = "dual_tracking_data"  # Dyadデータセットのディレクトリ
    save_dir_solo = "solo_tracking_data"  # Soloデータセットのディレクトリ
    output_dir = "trained_models"  # 学習済みモデルの保存先
    ...
    print(f"{'='*70}")
    print(f"複数エージェント×複数シード学習 (Dyad/Solo交互)")
    print(f"{'='*70}")
    print(f"エージェント数: {len(agents_config)}")
    print(f"各エージェントのシード数: {num_seeds}")
    print(f"総学習回数: {len(agents_config) * num_seeds}")
    print(f"偶数epoch: Dyadデータ, 奇数epoch: Soloデータ")
    print(f"{'='*70}\n")
    
    # 各エージェントのデータセットを読み込み（DyadとSolo両方）
    agents_datasets = {}
    for agent_name in agents_config.keys():
        print(f"\n=== {agent_name} のデータセット読み込み ===")
        
        # Dyadデータの読み込み
        print(f"\n[Dyadデータ]")
        dyad_train, dyad_val = load_dataset_from_directory(save_dir_dyad, f"{agent_name}_Dyad")
        if dyad_train is None or dyad_val is None:
            print(f"{agent_name} のDyadデータセット読み込みに失敗しました。")
            return
        
        # Soloデータの読み込み
        print(f"\n[Soloデータ]")
        solo_train, solo_val = load_dataset_from_directory(save_dir_solo, f"{agent_name}_Solo")
        if solo_train is None or solo_val is None:
            print(f"{agent_name} のSoloデータセット読み込みに失敗しました。")
            return
        
        agents_datasets[agent_name] = {
            'dyad_train': dyad_train,
            'dyad_val': dyad_val,
    ...
    for agent_name, config in agents_config.items():
        ...
        # 正規化器を作成（Dyad用とSolo用）
        print("\n[Dyad用normalizer作成]")
        normalizer_dyad = DataNormalizer()
        normalizer_dyad.fit(agents_datasets[agent_name]['dyad_train'])
        
        print("\n[Solo用normalizer作成]")
        normalizer_solo = DataNormalizer()
        normalizer_solo.fit(agents_datasets[agent_name]['solo_train'])
        ...
        for seed_idx, seed in enumerate(seeds):
            ...
            # 学習実行（DyadとSoloのデータセットと両方のnormalizerを渡す）
            train_losses, val_losses, train_acc, val_acc = train_agent_model(
                model=model,
                dyad_dataset_training=agents_datasets[agent_name]['dyad_train'],
                dyad_dataset_validation=agents_datasets[agent_name]['dyad_val'],
                solo_dataset_training=agents_datasets[agent_name]['solo_train'],
                solo_dataset_validation=agents_datasets[agent_name]['solo_val'],
                normalizer_dyad=normalizer_dyad,
                normalizer_solo=normalizer_solo,
                epochs=epochs,
            ...
            # 結果保存（両方のnormalizerを保存）
            save_model_and_results(
                model, normalizer_dyad, normalizer_solo,
                train_losses, val_losses, train_acc, val_acc,
                agent_name, output_dir, seed
            )
    ...
    print(f"{'='*70}")
    plot_multi_agent_multi_seed_results(all_results, output_dir, epochs)
    
    print(f"\n全ての学習が完了しました！結果は {output_dir} に保存されました。")
